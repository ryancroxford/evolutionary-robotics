...
class NEURON:
    ...
    def Update_Hidden_Or_Motor_Neuron(self, neurons, synapses):
        self.Set_Value(0.0)
        curr_neuron_name = self.Get_Name()
        for synapse in synapses:
            postsynaptic = synapse[1]
            presynaptic = synapse[0]
            if postsynaptic == self.Get_Name():
                synapse_weight = synapses[synapse].Get_Weight()
                presynaptic_value = neurons[presynaptic].Get_Value()
    ...
    def Allow_Presynaptic_Neuron_To_Influence_Me(self, synapse_weight, presynaptic_value):
        modifier = synapse_weight * presynaptic_value
        self.Add_To_Value(modifier)
